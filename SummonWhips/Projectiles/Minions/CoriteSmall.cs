...
namespace SummonWhips.Projectiles.Minions
{
	 ...
	 */
	public class CoriteSmall : ModProjectile
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Corite Small");
		...
		public override void AI()
		 {
			projectile.spriteDirection = projectile.direction - 2;
			Player player = Main.player[projectile.owner];

			#region Active check
			// This is the "active check", makes sure the minion is alive while the player is alive, and despawns if not
			if (player.dead || !player.active) {
				player.ClearBuff(ModContent.BuffType<Corite>());
			}
			if (player.HasBuff(ModContent.BuffType<Corite>())) {
