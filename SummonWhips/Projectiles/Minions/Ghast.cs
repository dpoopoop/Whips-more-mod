...
{
	internal class Ghast : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (target.active && projectile.Distance(target.Center) / 16 < SentryRange && projectile.ai[0] % Speed == 5) {
				...
				int damage = 40; //How much damage the projectile shot from the sentry will do.
				int type = mod.ProjectileType("GhastBall"); //The type of projectile the sentry will shoot. Use ModContent.ProjectileType<>() to fire a modded projectile.
				Projectile.NewProjectile(projectile.Center.X - 4f, projectile.Center.Y, direction.X, direction.Y, type, damage, 6f, projectile.owner);
                projectile.frame = 1;
			}
		}
	}
