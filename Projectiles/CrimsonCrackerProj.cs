...
{
	public class CrimsonCrackerProj : ModProjectile
	{
		// chain texture
		private const string ChainTexturePath = "SummonWhips/Gores/CrimsonCrackerChn";

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Crimson Cracker Projectile"); 
		}
		...
		{
			// PARTICLES
			var dust = Dust.NewDustDirect(projectile.position, projectile.width, projectile.height, 87, projectile.velocity.X * 0.4f, projectile.velocity.Y * 0.4f, 100, default, 1.5f);
			dust.noGravity = true;
			dust.velocity /= 2f;
			...
			if (projectile.ai[0] == 0f) {
				...
				float maxStretchLength = 114f; // number is max range - 6
