...
{
    public class NatureRod : ModItem
    {
		public override void SetStaticDefaults() {
			Tooltip.SetDefault(@"Summons a nature leaf over your head
'Only one nature leaf can be summoned at a time'");
		}
        public override void SetDefaults()
        { 
            item.mana = 14; //how much mana it uses.
			item.damage = 75; //the damage of the weapon. 
            ...
            item.shoot = mod.ProjectileType("NatureLeaf");  
            item.summon = true;    
            item.value = Item.sellPrice(silver: 700);
            item.buffType = ModContent.BuffType<Buffs.NatureLeaf>();
        }
