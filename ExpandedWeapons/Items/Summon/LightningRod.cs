...
namespace ExpandedWeapons.Items.Summon
{
    public class LightningRod : ModItem
    {
		public override void SetStaticDefaults() {
			Tooltip.SetDefault(@"Summons a lightning orb over your head
'Only one lightning orb can be summoned at a time'");
		}
        public override void SetDefaults()
        { 
            item.mana = 16; //how much mana it uses.
			item.damage = 110; //the damage of the weapon. 
            ...
            item.shoot = mod.ProjectileType("LightningOrb");  
            item.summon = true;    
            item.value = Item.sellPrice(silver: 1200);
            item.buffType = ModContent.BuffType<Buffs.LightningOrb>();
        }
