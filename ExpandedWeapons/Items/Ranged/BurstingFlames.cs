...
namespace ExpandedWeapons.Items.Ranged
{
	...
	public class BurstingFlames : ModItem
	{
		public override void SetStaticDefaults(){
			Tooltip.SetDefault(@"Shoots a big burst of flames.
'The slowest flamethrower, but with a lot of pop'");
		}

		public override void SetDefaults()
		{
			item.damage = 28; // The item's damage.
			item.ranged = true;
			item.width = 58;
			item.height = 32;
			item.useTime = 3;
			...
			item.shootSpeed = 5.5f; // How fast the flames will travel. Vanilla Flamethrower uses 7f and consequentially has less reach. item.shootSpeed and projectile.timeLeft together control the range.
			item.useAmmo = AmmoID.Gel; // Makes the weapon use up Gel as ammo
		}
		public override void AddRecipes() {
			ModRecipe recipe = new ModRecipe(mod);
			recipe.AddIngredient(534, 1);
			recipe.AddIngredient(8, 100);
		...
		public override bool ConsumeAmmo(Player player)
		{
			...
			return player.itemAnimation >= player.itemAnimationMax - 3; //set to usetime
		}

        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
			Vector2 muzzleOffset = Vector2.Normalize(new Vector2(speedX, speedY)) * 58f; //This gets the direction of the flame projectile, makes its length to 1 by normalizing it. It then multiplies it by 54 (the item width) to get the position of the tip of the flamethrower.
