...
{
    public class WhipOfMichiProj : ModProjectile
    {
        ...
        public override void SetStaticDefaults() {
            DisplayName.SetDefault("Michi Whip Projectile");
        }
        ...
        public override void AI() {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                Dust dust1 = Dust.NewDustPerfect(chainHeadPosition, 91, velocity, 150, Color.White, 1.5f);
            ...
            public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)                
        {
            ...
            player.AddBuff(ModContent.BuffType<Buffs.Crit>(), 4 * 60);
            player.AddBuff(ModContent.BuffType<Buffs.Tag>(), 4 * 60);
            player.GetModPlayer<ExpandedPlayer>().summonTagCrit = summonTagCrit;
            player.GetModPlayer<ExpandedPlayer>().summonTagDamage = summonTagDamage;
            target.AddBuff(ModContent.BuffType<Buffs.Pointy>(), 5 * 60);
            Main.PlaySound(mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/Crack"), (int)projectile.Center.X, (int)projectile.Center.Y);
            ...
            if (player.HasBuff(ModContent.BuffType<Buffs.Michi>()))
            {
            }
            else
            {
                Projectile.NewProjectile(target.Center.X, target.Center.Y - 54, 0, 0, mod.ProjectileType("MichigunShip"), damage / 2, 6f, Main.myPlayer, 0f, 0f); 
            }
            player.AddBuff(ModContent.BuffType<Buffs.Michi>(), 4 * 60);
		}
