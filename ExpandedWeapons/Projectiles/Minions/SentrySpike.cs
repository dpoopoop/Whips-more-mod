...
{
	internal class SentrySpike : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Sentry Spike"); //The english name of the projectile
		...
		public override void AI()
		{
			...
			int Speed = 120; //Fire rate
			float FireVelocity = 12f; //The velocity the sentry's shot projectile will travel. Slows down the closer the NPC is.
			...
			if (target.active && projectile.Distance(target.Center) / 16 < SentryRange && projectile.ai[0] % Speed == 5) {
				...
				Main.PlaySound(SoundID.Item, projectile.Center, 108); //Play a sound.

				int damage = projectile.damage; //How much damage the projectile shot from the sentry will do.
				int type = 24; //The type of projectile the sentry will shoot. Use ModContent.ProjectileType<>() to fire a modded projectile.
