    ...
    public class IceCrystal : ModProjectile
    {
        ...
        public override void Kill(int timeLeft) //on projectile death
        {
            ...
            for (int i = 0; i < 20; i++) 
            {
                ...
                int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 160, projectile.velocity.X * 1.2f, projectile.velocity.Y * 1.2f, 160, default(Color), 3.50f);   
        ...
        {       
            ...
            if (player.dead || !player.active) {
				player.ClearBuff(ModContent.BuffType<Buffs.IceCrystal>()); //handling with the summon buff.
			}
			if (player.HasBuff(ModContent.BuffType<Buffs.IceCrystal>())) {
            ...
            for (int i = 0; i < 200; i++) //this code below handles the aiming and shooting of the minion.
            {
                ...
                if (distance < 400f && !target.friendly && target.active && target.CanBeChasedBy()) //range is 400 pixels. 
                {
                    if (projectile.ai[0] > 60f) //Fires every second. (60 ticks)
                    {
                        ...
                        Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, shootToX, shootToY, mod.ProjectileType("IceBall"), damage, 4f, Main.myPlayer, 0f, 0f); 
