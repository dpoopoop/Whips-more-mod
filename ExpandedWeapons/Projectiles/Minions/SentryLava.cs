...
{
	internal class SentryLava : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Sentry Lava"); //The english name of the projectile
		...
		public override void AI()
		{
			...
			int SentryRange = 36; //The sentry's range
			int Speed = 100; //Fire rate
			float FireVelocity = 16f; //The velocity the sentry's shot projectile will travel. Slows down the closer the NPC is.
			...
			if (target.active && projectile.Distance(target.Center) / 16 < SentryRange && projectile.ai[0] % Speed == 5) {
				...
				Main.PlaySound(SoundID.Item, projectile.Center, 20); //Play a sound.

				int damage = projectile.damage; //How much damage the projectile shot from the sentry will do.
				int type = 664; //The type of projectile the sentry will shoot. Use ModContent.ProjectileType<>() to fire a modded projectile.
