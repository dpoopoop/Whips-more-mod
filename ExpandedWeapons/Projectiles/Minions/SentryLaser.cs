...
{
	internal class SentryLaser : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Sentry Laser"); //The english name of the projectile
		...
		public override void AI()
		{
			...
			/*for (int k = 0; k < 1; k++) {
				int dust = Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 247, projectile.oldVelocity.X * 0.5f, projectile.oldVelocity.Y * 0.5f);
				Main.dust[dust].noGravity = true; //Disable the dust gravity.
				Main.dust[dust].velocity *= 0.8f; //Dust velocity.
			}*/

			int SentryRange = 45; //The sentry's range
			int Speed = 26; //Fire rate
			...
			if (target.active && projectile.Distance(target.Center) / 16 < SentryRange && projectile.ai[0] % Speed == 5) {
				...
				Main.PlaySound(SoundID.Item, projectile.Center, 12); //Play a sound.

				int damage = projectile.damage; //How much damage the projectile shot from the sentry will do.
				int type = mod.ProjectileType("MinionBeam"); //The type of projectile the sentry will shoot. Use ModContent.ProjectileType<>() to fire a modded projectile.
				Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y - 26f, direction.X, direction.Y, type, damage, 6f, projectile.owner);
			}
		}
	}
} 
