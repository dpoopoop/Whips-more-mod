...
{
	internal class SentryMachine : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Sentry Machine"); //The english name of the projectile
		...
		public override void AI()
		{
			...
			int SentryRange = 50; //The sentry's range
			...
			if (target.active && projectile.Distance(target.Center) / 16 < SentryRange && projectile.ai[0] % Speed == 5) {
				...
				Main.PlaySound(SoundID.Item, projectile.Center, 11); //Play a sound.

				int damage = projectile.damage; //How much damage the projectile shot from the sentry will do.
				int type = 14; //The type of projectile the sentry will shoot. Use ModContent.ProjectileType<>() to fire a modded projectile.
				Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y - 22f, direction.X, direction.Y, type, damage, 6f, projectile.owner);
			}
		}
	}
} 
