...
{
	internal class SentryPlantera : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Sentry Plantera"); //The english name of the projectile
		...
		public override void AI()
		{
			...
			for (int k = 0; k < 1; k++) {
				int dust = Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 2, projectile.oldVelocity.X * 0.5f, projectile.oldVelocity.Y * 0.5f);
			...
			int SentryRange = 55; //The sentry's range
			int Speed = 20; //Fire rate
			float FireVelocity = 18f; //The velocity the sentry's shot projectile will travel. Slows down the closer the NPC is.
			...
			if (target.active && projectile.Distance(target.Center) / 16 < SentryRange && projectile.ai[0] % Speed == 5) {
				...
				Main.PlaySound(SoundID.Item, projectile.Center, 17); //Play a sound.

				int damage = projectile.damage; //How much damage the projectile shot from the sentry will do.
				int type = mod.ProjectileType("BetterSeed"); //The type of projectile the sentry will shoot. Use ModContent.ProjectileType<>() to fire a modded projectile.
				Projectile.NewProjectile(projectile.Center.X - 0f, projectile.Center.Y, direction.X, direction.Y, type, damage, 6f, projectile.owner);
			}
		}
	}
} 
