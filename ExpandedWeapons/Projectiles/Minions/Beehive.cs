    ...
    public class Beehive : ModProjectile
    {
        ...
        public override void Kill(int timeLeft) //on projectile death
        {
            ...
            for (int i = 0; i < 20; i++) 
            {
                ...
                int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 87, projectile.velocity.X * 1.2f, projectile.velocity.Y * 1.2f, 160, default(Color), 3.50f);   
        ...
        {       
            ...
            if (player.dead || !player.active) {
				player.ClearBuff(ModContent.BuffType<Buffs.Beehive>()); //handling with the summon buff.
			}
			if (player.HasBuff(ModContent.BuffType<Buffs.Beehive>())) {
            ...
            for (int i = 0; i < 200; i++) //this code below handles the aiming and shooting of the minion.
            {
                ...
                {
                    if (projectile.ai[0] > 75f) //Fires every second. (60 ticks)
                    {
                        ...
                        Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, shootToX, shootToY, 181, damage, 4f, Main.myPlayer, 0f, 0f); 
                        Main.PlaySound(2, (int)projectile.position.X, (int)projectile.position.Y, 11); //28 is the sound
                        projectile.ai[0] = 0f;
                    }
                }
            }
