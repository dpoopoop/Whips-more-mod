...
{
	internal class SentryDispenser : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Sentry Dispenser"); //The english name of the projectile
		...
		public override void AI()
		{
			...
			if (target.active && projectile.Distance(target.Center) / 16 < SentryRange && projectile.ai[0] % Speed == 5) {
				...
				Main.PlaySound(SoundID.Item, projectile.Center, 5); //Play a sound.

				int damage = projectile.damage; //How much damage the projectile shot from the sentry will do.
				int type = mod.ProjectileType("BetterArrow"); //The type of projectile the sentry will shoot. Use ModContent.ProjectileType<>() to fire a modded projectile.
