...
{
	public class OreoBeamMk2 : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / OreoHoldout.NumBeams;
		...
		private const float InnerBeamOpacityMultiplier = 0.05f;
		...
		private const float BeamColorSaturation = 0.025f;
		...
		{
			DisplayName.SetDefault("Oreo Beam Mk2");
		}
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<OreoBeamMk2>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<OreoHoldout>())
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / OreoHoldout.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > OreoHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - OreoHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * OreoHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / OreoHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
