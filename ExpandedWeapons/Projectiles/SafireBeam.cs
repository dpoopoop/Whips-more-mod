...
{
	public class SafireBeam : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / SafireHoldout.NumBeams;
		...
		private const float MaxBeamSpread = 0.3f;
		...
		private const float InnerBeamOpacityMultiplier = 0.4f;
		...
		private const float BeamHueVariance = 0.1f;
		...
		{
			DisplayName.SetDefault("Safire Beam");
		}
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<SafireBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<SafireHoldout>())
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / SafireHoldout.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > SafireHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - SafireHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * SafireHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / SafireHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
