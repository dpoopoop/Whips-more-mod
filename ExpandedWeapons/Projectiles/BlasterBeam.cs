...
namespace ExpandedWeapons.Projectiles
{
	public class BlasterBeam : ModProjectile
	{
		...
		private const float MaxBeamSpread = 0.85f;
		...
		private const float InnerBeamOpacityMultiplier = 0.3f;
		...
		private const float BeamColorHue = 0.45f;
		private const float BeamHueVariance = 0.2f;
		private const float BeamColorSaturation = 0.66f;
		private const float BeamColorLightness = 0.55f;
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<BlasterBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<BlasterHoldout>())
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / BlasterHoldout.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > BlasterHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - BlasterHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * BlasterHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / BlasterHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
