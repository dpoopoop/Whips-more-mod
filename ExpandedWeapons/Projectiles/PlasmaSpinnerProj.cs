...
{
    public class PlasmaSpinnerProj : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Plasma Spinner");
        ...
        {
            {
                Projectile.NewProjectileDirect(target.Center, Vector2.Zero, ModContent.ProjectileType<PlasmaExplosion>(), damage / 2, 1f, projectile.owner, 0f, 1f);
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                var dust = Dust.NewDustDirect(target.position, target.width, target.height, 257);
                dust.scale = 2f;
                dust.noGravity = true;
        ...
        public override void AI()
        {
            ...
            int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, 257);  //this is the dust that this projectile will spawn
            Main.dust[dust].velocity /= 1f;
            Main.dust[dust].scale = 2f;
            Main.dust[dust].noGravity = true;
            projectile.usesLocalNPCImmunity = true;
            projectile.localNPCHitCooldown = 4; //hit speed (how fast it attacks)
        }
