...
{
	public class TyphoonCloud : ModProjectile
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Typhoon Cloud");
		...
		public override void AI()
		{
			...
			Projectile.NewProjectile(projectile.Center.X + MathHelper.Lerp(-32f, 32f, (float)Main.rand.NextDouble()), projectile.Center.Y + 32, 0, 6,  mod.ProjectileType("Betterrain"), damage - 20, 2f, Main.myPlayer, 0f, 0f); 
			}
			...
			for (int i = 0; i < 200; i++) //this code below handles the aiming and shooting of the minion.
            {
                ...
                if (distance < 600f && !target.friendly && target.active && target.CanBeChasedBy()) //range is 400 pixels. 
                {
                    if (projectile.ai[0] > 60f) //Fires every second. (60 ticks)
                    {
                        ...
                        Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, shootToX, shootToY, mod.ProjectileType("Bolt"), damage + 60, 4f, Main.myPlayer, 0f, 0f); 
                        Main.PlaySound(2, (int)projectile.position.X, (int)projectile.position.Y, 66); //28 is the sound
                        projectile.ai[0] = 0f;
                    }
                }
            }
