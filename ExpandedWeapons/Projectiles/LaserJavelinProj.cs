...
{
	public class LaserJavelinProj : ModProjectile
	{
		...
		public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection) {
			...
			target.AddBuff(ModContent.BuffType<Buffs.LaserJavelinBuff>(), 600); // Adds the ExampleJavelin debuff for a very small DoT
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
				    ...
				    && currentProjectile.modProjectile is LaserJavelinProj javelinProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
		...
		private void NormalAI()
		{
		     ...
		     for (int i = 0; i < 200; i++) //this code below handles the aiming and shooting of the minion.
            {
				...
				{
				Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, shootToX, shootToY, mod.ProjectileType("Betterlaser"), damage / 2, 4f, Main.myPlayer, 0f, 0f); 
