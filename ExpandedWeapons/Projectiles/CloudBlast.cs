...
namespace ExpandedWeapons.Projectiles
{
    ...
    public class CloudBlast : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("Cloud Blast");
        }
        ...
        private void AICharge()
        {
            ...
            if (projectile.ai[0] == 0) //initial firing
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    d1 = Dust.NewDust(endPos + new Vector2(projectile.width / 2, projectile.height / 2), 0, 0, 66, 0f, 0f, 100, Color.White, 1f);
                    Main.dust[d1].noGravity = true;
                    Main.dust[d1].velocity *= 0.4f;
                }
            }
        ...
        private void AILaunch()
        {
            ...
            //purpel stuph
            int d1 = Dust.NewDust(projectile.position + new Vector2(projectile.width / 2f, projectile.height / 2f), 1, 1, 66, 0f, 0f, 100, Color.White, 1f);
            Main.dust[d1].noGravity = true;
        ...
        public override void Kill(int timeLeft)
        {
            for (int j = 0; j < 40; j++)
            {
                int d2 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 66, (j / 90f) * -projectile.velocity.X, (j / 90f) * -projectile.velocity.Y, 100, Color.White, 1.2f);
        ...
        private void drawLaser(SpriteBatch spritebatch, Vector2 centre)
        {
            ...
            if (projectile.ai[0] > 0)
            {
                ...
                Utils.DrawLaser(
                    spritebatch,
                    mod.GetTexture("Projectiles/CloudBlast_Beam"),
                    start - Main.screenPosition,
                    end - Main.screenPosition,
                    new Vector2(projectile.ai[1]),
                    new Utils.LaserLineFraming(CloudLaser)); //uses delegate (see method below)
            }
            
            
        }
        //define which frames are used in each stage (0 = start, 1 = mid, 2 = end
        private void CloudLaser(int stage, Vector2 currentPosition, float distanceLeft, Rectangle lastFrame, out float distCovered, out Rectangle frame, out Vector2 origin, out Color color)
        {
            color = Color.White;
			if (stage == 0)
			{
