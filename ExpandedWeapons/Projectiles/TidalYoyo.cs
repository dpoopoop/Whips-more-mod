...
{
	public class TidalYoyo : ModProjectile
	{
		...
		public override void SetStaticDefaults() {
			...
			ProjectileID.Sets.YoyosLifeTimeMultiplier[projectile.type] = 35f;
		...
		public override void PostAI() 
		{
			...
			if (Main.rand.NextBool()) 
			{
				...
				if (waterCooldown == 15)
				{
			Projectile.NewProjectile(projectile.position.X, projectile.position.Y, MathHelper.Lerp(-3f, 3f, (float)Main.rand.NextDouble()), MathHelper.Lerp(-5f, 5, (float)Main.rand.NextDouble()), mod.ProjectileType("TidalBeam"), damage / 3, 0f, Main.myPlayer);
			Projectile.NewProjectile(projectile.position.X, projectile.position.Y, MathHelper.Lerp(-3f, 3f, (float)Main.rand.NextDouble()), MathHelper.Lerp(-5f, 5, (float)Main.rand.NextDouble()), mod.ProjectileType("TidalBeam"), damage / 3, 0f, Main.myPlayer);
			waterCooldown = 0;
			    }
			}
